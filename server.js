...
function leadFromOverpassElement(e, fallbackCategory = 'Não informado') {
  const tags = e.tags || {};
  return {
    source: 'OpenStreetMap/Overpass',
    overpassId: e.id,
    name: tags.name || 'Não Encontrado',
    category: fallbackCategory,
    address: tags['addr:full'] || [tags['addr:street'], tags['addr:housenumber']].filter(Boolean).join(', ') || 'Não Encontrado',
    email: tags.email || tags['contact:email'] || 'Não Encontrado',
    phone: tags.phone || tags['contact:phone'] || 'Não Encontrado',
    site: tags.website || tags['contact:website'] || 'Não Encontrado',
    social: tags.facebook || tags.instagram || tags['contact:facebook'] || tags['contact:instagram'] || 'Não Encontrado',
...
async function fetchOverpassCompanies({ lat, lon, radius, category, term, limit }) {
  const overpassTag = categoryToOverpass(category);
  const safeTerm = escapeOverpassRegex(term || '');
  const nameFilter = safeTerm ? `["name"~"${safeTerm}", i]` : '';
  const query = `[out:json][timeout:35];(node[${overpassTag}]${nameFilter}(around:${radius},${lat},${lon});way[${overpassTag}]${nameFilter}(around:${radius},${lat},${lon});relation[${overpassTag}]${nameFilter}(around:${radius},${lat},${lon}););out center tags ${limit};`;
  const response = await requestJson('https://overpass-api.de/api/interpreter', {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(`Falha Overpass: ${details.slice(0, 180)}`);
  }
  const data = await response.json();
  const elements = data?.elements || [];
  const unique = new Map();
  for (const e of elements) {
    const lead = leadFromOverpassElement(e, category || 'Não informado');
    if (!lead.lat || !lead.lon) continue;
    const key = `${lead.name}|${lead.lat.toFixed(6)}|${lead.lon.toFixed(6)}`;
...
app.get('/api/empresas/search', async (req, res) => {
  try {
    ...
    const rad = Math.min(30000, Math.max(500, Number(radius || 5000)));
    ...
    if (!Number.isFinite(latNum) || !Number.isFinite(lonNum)) {
      ...
      if (!coords) return res.status(400).json({ message: 'Não foi possível localizar a região informada.' });
      latNum = coords.lat;
      lonNum = coords.lon;
      resolvedLocation = coords.label;
    }

    let results = await fetchOverpassCompanies({
      lat: latNum,
      lon: lonNum,
      radius: rad,
      category,
    ...
    if (String(hasPhone) === 'true') results = results.filter((r) => r.phone && r.phone !== 'Não Encontrado');
    if (String(hasEmail) === 'true') results = results.filter((r) => r.email && r.email !== 'Não Encontrado');
    if (String(hasWebsite) === 'true') results = results.filter((r) => r.site && r.site !== 'Não Encontrado');

    res.json({
      source: 'OpenStreetMap/Overpass + Nominatim',
      query: { q, category, location: resolvedLocation, lat: latNum, lon: lonNum, radius: rad, limit: maxLimit },
      total: results.length,
      items: results,
    });
  } catch (error) {
    res.status(502).json({ message: 'Erro ao buscar empresas reais.', details: error.message });
  }
});

app.get('/api/empresas/similares/:leadId', async (req, res) => {
  try {
    const lead = await LeadCapture.findById(req.params.leadId);
    if (!lead) return res.status(404).json({ message: 'Lead não encontrado.' });
    if (!Number.isFinite(Number(lead.lat)) || !Number.isFinite(Number(lead.lon))) {
      return res.status(400).json({ message: 'Lead sem coordenadas para busca de similares.' });
    }

    const termSeed = String(lead.name || '').split(' ').filter((w) => w.length > 3)[0] || '';
    const limit = Math.min(30, Math.max(1, Number(req.query.limit || 12)));
    const radius = Math.min(20000, Math.max(500, Number(req.query.radius || 5000)));

    const items = await fetchOverpassCompanies({
      lat: Number(lead.lat),
      lon: Number(lead.lon),
      radius,
    ...
    const filtered = items.filter((i) => !(Math.abs(i.lat - Number(lead.lat)) < 0.00001 && Math.abs(i.lon - Number(lead.lon)) < 0.00001)).slice(0, limit);

    res.json({
      source: 'OpenStreetMap/Overpass',
      basedOn: { id: lead._id, name: lead.name, category: lead.category, lat: lead.lat, lon: lead.lon },
      total: filtered.length,
      items: filtered,
    });
  } catch (error) {
    res.status(502).json({ message: 'Erro ao buscar empresas parecidas.', details: error.message });
  }
});
